...
describe('createAuthenticator', () => {
  ...
  test('should fail when creating authenticator with invalid userPoolSignInEndpoint', () => {
    params.userPoolSignInEndpoint = 'not-ok';
    expect(() => new Authenticator(params)).toThrow('userPoolSignInEndpoint');
  });
});
...
describe('handle', () => {
  ...
  test('should redirect to auth domain userPoolSignInEndpoint if defined', () => {
    authenticator._userPoolSignInEndpoint = 'login';
    authenticator._getVerifiedToken.mockImplementationOnce(() => { throw new Error();});
    return expect(authenticator.handle(getCloudfrontRequest())).resolves.toEqual(
      {
        status: 302,
        headers: {
          location: [{
            key: 'Location',
            value: 'https://my-cognito-domain.auth.us-east-1.amazoncognito.com/login?redirect_uri=https://d111111abcdef8.cloudfront.net&response_type=code&client_id=123456789qwertyuiop987abcd&state=/lol',
          }],
        },
      }
    )
