...
/**
 * Allowed values that can be passed to params.userPoolSignInEndpoint.
 */
const allowedSignInEndpointValues = [
  'authorize',
  'login',
  'oauth2/authorize',
];
...
class Authenticator {
  constructor(params) {
    ...
    this._userPoolSignInEndpoint = params.userPoolSignInEndpoint || 'authorize';
  ...
  _verifyParams(params) {
    ...
    if (params.userPoolSignInEndpoint && !allowedSignInEndpointValues.includes(params.userPoolSignInEndpoint)) {
      throw new Error(`Expected params.userPoolSignInEndpoint to be one of ${allowedSignInEndpointValues.toString()}`);
    }
  }
  ...
  async handle(event) {
    ...
    try {
      ...
      if (requestParams.code) {
        ...
        const userPoolUrl = `https://${this._userPoolDomain}/${this._userPoolSignInEndpoint}?redirect_uri=${redirectURI}&response_type=code&client_id=${this._userPoolAppId}&state=${redirectPath}`;
